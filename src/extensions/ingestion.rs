//! Chunk enrichment extension point.
//!
...
//! A different enricher registered here changes the text each chunk is
...
/// a trait so a downstream crate can register a different implementation
/// without this one depending on its code.
#[async_trait]
pub trait ChunkEnricher: Send + Sync {
    async fn enrich(&self, document_text: &str, chunks: &[Chunk]) -> Result<Vec<String>>;
}

/// The default: the regex-based structural-heading injection already
/// shipped and tested (see rag::chunker::{detect_headings,
/// inject_heading_context}). Not a no-op — this crate already has real,
/// generic chunk enrichment — and with nothing else registered, behavior is
